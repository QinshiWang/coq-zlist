...
Definition list_repeat {A} (n : nat) (x : A) :=
  repeat x n.

Lemma length_list_repeat : forall {A} (n : nat) (x : A),
  length (list_repeat n x) = n.
Proof.
...
induction n; intros.
constructor.
unfold list_repeat in *; constructor; auto.
Qed.
...
Proof.
  ...
  destruct (Z.eq_dec i i); try lia; trivial.
Qed.
...
Proof.
  intros. rewrite (upd_Znth_lookup' _ _ (eq_refl _)); trivial.
  destruct (Z.eq_dec i j); try lia; trivial.
Qed.
